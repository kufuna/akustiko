    ...
    for(var i = 0; i < this.files.length; i++) {
      ...
      reader.onload = (function(theFile, ord) {
        return function(e) {
          ...
          // chosenImage = $('<img />').prop({  draggable: false, src: e.target.result, title: escape(theFile.name) }).click(expandPoster)
          chosenImage = $('<img />').prop({  draggable: false, src: e.target.result, title: escape(theFile.name) });
          cropInput = $('<input />').prop({ type: 'hidden', name: cropName })
          preview = $('<div>').addClass('photo-preview').data({ ord: ord, new: true }).append(chosenImage).append(cropInput)
