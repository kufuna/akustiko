<div class="formRow">
  ...
  <div class="<?= $cfg->preview_name ?> custom-floor-container-prev">
      <?php
      ...
      <div class="bilding-photo-preview" id="building_photo_parent_<?= $cfg->id ?>"<?= $previewStyle ?>>
        ...
        <svg id="svg_<?= $cfg->id ?>" class="building-svg"<?= $previewStyle ?>>
          ...
          ?>
            <polygon fill="rgba(81,124,218,.8)" class="fadeable-svg-polygon" data-floornumber="<?= $floor->{$cfg->floor_number_col} ?>"  data-floorblock="<?= $floor->{$cfg->floors_key} ?>" data-coords="<?= $floor->coords ?>" points="<?= $floor->coords ?>"></polygon>
          <?php
              }
            }
          ?>
        ...
        <div class="floor-number-preview" id="fblock_<?= $cfg->id ?>"></div>
      </div>
...
<script>
  $(function() {
    ...
    function setFloorNumber() {
      ...
      // var fblock = prompt('Enter block number', $(this).data().floorblock);
      $(this).data().floornumber = fn;
      // $(this).data().floorblock = fblock;

      $('#fnumber_<?= $cfg->id ?>').text(fn);

      // $('#fblock_<?= $cfg->id ?>').text(fblock);
      serializeFloors();
    }
    ...
    function serializeFloors() {
      ...
      console.log(data.floorblock)
        
        result[data.floornumber+'_'+data.floorblock] = data;
      }
