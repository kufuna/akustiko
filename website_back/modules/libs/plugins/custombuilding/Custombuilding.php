<?php
class P_Custombuilding extends Plugin {
  ...
  public function getData($data) {
    ...
    if($floorCoords && $this->currentValue) {
      cn_db_update_building(array( $cfg->floors_key => 0 ), 'building_floors', $cfg->floors_key, $this->currentValue->id, false);

      foreach($floorCoords as $k => $v) {
      	$v->floorblock = !isset($v->floorblock) ? 0 : $v->floorblock;
        $data = array( 'coords' => $v->coords, $cfg->floors_key => $v->floorblock, $cfg->floors_key => $this->currentValue->id );
        $data[$cfg->floor_number_col] = $v->floornumber;
        $new_floors_id = cn_db_insert($data, $cfg->floors_table, false, true);

        $data1 = array( $cfg->floors_key => $this->currentValue->id, 'new_floors_id' => $new_floors_id );
        cn_db_insert($data1, 'building_floors');
